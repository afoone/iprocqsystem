    ...
    protected Response preparationReport(HttpRequest request) {
        ...
        try {
            ...
            if ("/ru/apertum/qsystem/reports/web/reportList.html".equals(res)) {
                ...
                final String cookie = "<META HTTP-EQUIV=\"Set-Cookie\" CONTENT=\"username=" + URLEncoder.encode(usr, "utf-8") + "\">\n<META HTTP-EQUIV=\"Set-Cookie\" CONTENT=\"parol=" + URLEncoder.encode(pwd, "utf-8") + "\">";
                result = new String(result, "UTF-8").replaceFirst(Uses.ANCHOR_COOCIES, cookie).getBytes("UTF-8");
            }
        } catch (IOException ex) {
            throw new ReportException("Ошибка чтения ресурса для диалогового выбора отчета. " + ex);
        }
        final Response response = new Response(result, logonPostParams);
        return response;
    }

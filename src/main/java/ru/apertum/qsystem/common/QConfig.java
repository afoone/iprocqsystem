...
public final class QConfig {
    ...
    private Options initOptions(QModule type) {
        ...
        if (type.isUnknown() || type.isClient() || type.isDesktop()) {
            ...
            options.addOption("nty", KEY_NO_TRAY, false, "Do not to hide to tray.");
        }

        if (type.isUnknown() || type.isClient() || type.isReception() || type.isWelcome() || type.isDesktop()) {
        ...
        if (type.isUnknown() || type.isClient() || type.isReception() || type.isAdminApp() || type.isWelcome() || type.isUB() || type.isDesktop()) {
            o = new Option("httpp", KEY_HTTP_PROTOCOL_POST, true, "Use HTTP as protocol for transporting POST commands from clients to server.");
            o.setArgName("port");
            options.addOption(o);

            o = new Option("httpg", KEY_HTTP_PROTOCOL_GET, true, "Use HTTP as protocol for transporting GET commands from clients to server.");
            o.setArgName("port");
            options.addOption(o);

            o = new Option("px", KEY_PROXY, true, "Proxy settings hostName:port. Example 127.0.0.1:2130");
            o.setArgName("hostName:port");
            options.addOption(o);

            o = new Option("nopx", KEY_NO_PROXY, false, "Do not use proxy.");
            options.addOption(o);

            o = new Option("f", KEY_FONT, true, "Font for GUI. 'no' = default. Use pattern like name-type-size. In which style is one of the four case-insensitive strings: \"PLAIN\", \"BOLD\", \"BOLDITALIC\", or \"ITALIC\", and pointsize is a positive decimal integer representation of the point size. For example, if you want a font that is Arial, bold, with a point size of 18, you would call this method with: \"Arial-BOLD-18\". For ex.: Tahoma-Plain-14 or Arial-Bold-12 or Tahoma-Italic-16");
            o.setArgName("name-type-size");
            options.addOption(o);
        }
    ...
    public boolean isLogInfo() {
        return line.hasOption(KEY_LOG_INFO) || config.getBoolean(KEY_LOG_INFO, false);
    }
    ...
    public HttpRequestType getHttpRequestType() {
        return getHttpProtocol() > 0 ? (line.hasOption(KEY_HTTP_PROTOCOL_POST) || config.getInt(KEY_HTTP_PROTOCOL_POST, 0) > 0 ? HttpRequestType.POST : HttpRequestType.GET) : HttpRequestType.NOT_USE_HTTP;
    }
    ...
    public boolean showHidenProps() {
        return line.hasOption(KEY_USE_HIDE_PROPS)
                ? true
                : config.getBoolean(KEY_USE_HIDE_PROPS, false);
    }
    ...
    public boolean doNotUseProxy() {
        return line.hasOption(KEY_NO_PROXY)
                ? true
                : config.getBoolean(KEY_NO_PROXY, false);
    }
    ...
    public boolean needVoice() {
        return line.hasOption(KEY_VOICE)
                ? true
                : config.getBoolean(KEY_VOICE, false);
    }
    ...
    public boolean doNotUseTray() {
        return line.hasOption(KEY_NO_TRAY)
                ? true
                : config.getBoolean(KEY_NO_TRAY, false);
    }

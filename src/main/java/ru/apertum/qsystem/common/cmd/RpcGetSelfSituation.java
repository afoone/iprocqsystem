...
public class RpcGetSelfSituation extends JsonRPC20 {
    ...
    public static class CustomerShort {
        ...
        public CustomerShort(Long id, String number, String data, Integer waiting) {
            this.id = id;
            this.number = number;
            this.data = data;
    ...
    public static class SelfService {
         ...
         * @param flexyInvitation можно вызывать прямо из очереди к услуге
         */
        public SelfService(QService service, int countWait, int priority, boolean flexy, boolean flexyInvitation) {
            this.serviceName = service.getName();
            this.countWait = countWait;
            this.duration = service.getDuration();
            this.priority = priority;
            this.flexy = flexy;
            this.flexyInvitation = flexyInvitation;
            this.roll = service.getStatus() == 5;
            this.id = service.getId();
            this.line = new LinkedList<>();
            service.getClients().stream().forEach((cu) -> {
                final CustomerShort sp = new CustomerShort(cu.getId(), cu.getFullNumber(), cu.getInput_data(), cu.getWaitingMinutes());
                line.addLast(sp);
            });
        }
        ...
        @Expose
        @SerializedName("flexy_invite")
        private boolean flexyInvitation;

        public boolean isFlexyInvitation() {
            return flexyInvitation;
        }

        public void setFlexyInvitation(boolean flexyInvitation) {
            this.flexyInvitation = flexyInvitation;
        }
        ...
        @Expose
        @SerializedName("line")
        private LinkedList<CustomerShort> line;

        public LinkedList<CustomerShort> getLine() {
            return line;
        }

        public void setLine(LinkedList<CustomerShort> line) {
            this.line = line;
        }

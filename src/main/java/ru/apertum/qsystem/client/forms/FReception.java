...
public class FReception extends javax.swing.JFrame {
    ...
    final private transient IClientNetProperty netProperty;
    ...
    private void init() {
        ...
        setTitle(getTitle() + " " + Uses.getLocaleMessage("project.name")); //NOI18N
    ...
    private static class PreTableModel extends AbstractTableModel {
        ...
        final private transient RpcGetGridOfDay.GridDayAndParams greed;
            ...
            switch (columnIndex) {
                ...
                case 1:
                    if (greed.get(rowIndex).getShadow() == null) {
                        ...
                        final int mnt = Math.toIntExact(((new Date().getTime() - greed.get(rowIndex).getShadow().getFinTime().getTime()) / 1000 / 60));
                        ...
                        final int mnt = Math.toIntExact(((new Date().getTime() - greed.get(rowIndex).getShadow().getStartTime().getTime()) / 1000 / 60));
                        final boolean toolong = (mnt > standards.getDowntimeMax());
    ...
    private void initComponents() {
        ...
        popupServiceTree.addPopupMenuListener(new javax.swing.event.PopupMenuListener() {
            ...
            public void popupMenuWillBecomeVisible(javax.swing.event.PopupMenuEvent evt) {
                popupServiceTreePopupMenuWillBecomeVisible(evt);
            }
        });

        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance().getContext().getActionMap(FReception.class, this);
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FReception.class);
        ...
        popupServiceTree.add(miInitRoll);
    ...
    private void buttonRefreshUserActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonRefreshUserActionPerformed
        if (listUsers.getLastVisibleIndex() != -1) {
            ...
            for (SelfService serv : plan.getSelfservices()) {
                ...
                tempAll = tempAll + "<span style='color:red'> - " + serviceName + ": " + count + people
            ...
            if (plan.getCustomer() != null) {
                ...
                if (plan.getShadow() == null) {
                    ...
                    final int mnt = Math.toIntExact((new Date().getTime() - plan.getShadow().getFinTime().getTime()) / 1000 / 60);
            ...
            if (fc.getAcust().getAdvanceTime() == null) {
                ...
                if (res != null) {
                    ...
                    if (res.getMethod() == null) {// костыль. тут приедет текст запрета если нельзя встать в очередь
                        ...
                        new Thread(() -> {
                            FWelcome.printTicket(res.getResult(), ((QService) treeServices.getModel().getRoot()).getTextToLocale(QService.Field.NAME));
                        }).start();
                        preRegChange(true);
    ...
    private void miInitRollActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_miInitRollActionPerformed
        ...
        if (service != null && !service.isRoot()) {
            ...
            if (dialog.isOk()) {
                NetCommander.initRoll(netProperty, service.getId(), dialog.getFirstNumber(), dialog.getLastNumber(), dialog.getCurrentNumber());
            }
        }
    }//GEN-LAST:event_miInitRollActionPerformed

    private void popupServiceTreePopupMenuWillBecomeVisible(javax.swing.event.PopupMenuEvent evt) {//GEN-FIRST:event_popupServiceTreePopupMenuWillBecomeVisible
        final QService service = (QService) treeServices.getLastSelectedPathComponent();
        miInitRoll.setEnabled(service != null && service.getStatus() == 5);
        menuItemStand.setEnabled(service != null && service.getStatus() != 5);
    ...
    public static void main(String args[]) {
        QLog.initial(args, QModule.reception);
    ...
    private class QCustomer2 implements IidGetter {
        ...
        public String toString() {
            ...
            return "<html>" + nom + ".  " + customer.getFullNumber() + (customer.getPostponedStatus().isEmpty() ? "" : " " + customer.getPostponedStatus()

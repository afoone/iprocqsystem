...
public class FResponseDialog extends javax.swing.JDialog {
     ...
     * @param modal
     */
    public FResponseDialog(java.awt.Frame parentFrame, boolean modal) {
    ...
    private void showLevel(QRespItem level) {
        ...
        if (level.isLeaf() || level.getChildCount() == 1) {
            ...
            int cols = 1;
            int rows = level.getChildCount() + (level.getInput_caption().trim().isEmpty() ? 0 : 1);

            panelMain.setLayout(new GridLayout(rows < 5 ? 5 : rows, cols, delta, delta / 2));
            if (!level.getInput_caption().trim().isEmpty()) {
                final FResponseDialog.RespButton button = new FResponseDialog.RespButton(parentFrame, FResponseDialog.level, null);
    ...
    private static final class RespButton extends JButton {
        ...
        public RespButton(Frame parent, QRespItem item, String resourceName) {
            ...
            setText(!item.isLeaf() && resourceName == null ? item.getInput_caption() : item.getHTMLText());
            ...
            addActionListener((ActionEvent e) -> {
                if (el.isLeaf()) {
                    ...
                    if (el.getInputRequired()) {
                        inputData = FInputDialog.showInputDialog(parent, true, FWelcome.netProperty, false, WelcomeParams.getInstance().delayBack, el.getInput_caption());
                    }

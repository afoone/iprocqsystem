...
public class FAdvanceCalendar extends javax.swing.JDialog {
    ...
    public static QAdvanceCustomer showCalendar(Frame parent, boolean modal, INetProperty netProperty, QService service, boolean fullscreen, int delay, long advCustomer, String inputData, String comments) {
        ...
        if (advanceCalendar.showWeek(new Date())) {
            if (!(QConfig.cfg().isDebug() || QConfig.cfg().isDemo()) && fullscreen) {
                ...
                advanceCalendar.setSize(FWelcome.getScreenHorizontal(), FWelcome.getScreenVertical());
                Uses.setLocation(advanceCalendar, parent);
                advanceCalendar.setVisible(true);
            }
        }
        return result;
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FAdvanceCalendar.class);
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance().getContext().getActionMap(FAdvanceCalendar.class, this);
    ...
    private void showWeek(GridAndParams res, GregorianCalendar gc) {
        ...
        if (WelcomeParams.getInstance().advanceCloseButtonEmpty) {
            buttonClose.setText("");
            Container c = buttonClose.getParent();
            c.remove(buttonClose);
            buttonClose.setSize(buttonClose.getHeight(), buttonClose.getHeight());
            c.add(buttonClose);
        }
    }
    ...
    private void printDayWeek(JPanel panel, GridAndParams res, int weekDay) {
        ...
        for (Date dd : res.getTimes()) {
            ...
            if (ii == weekDay && f && gc.getTime().after(gc_now.getTime())) {
                panel.add(new QAvancePanel(date -> {
                    if (clockBack.isActive()) {
                        clockBack.stop();
                    }
    ...
    private void printCaptionWeek(JLabel label, int day, GregorianCalendar gc) {
        ...
        if (now.get(GregorianCalendar.DAY_OF_MONTH) == gc.get(GregorianCalendar.DAY_OF_MONTH)
            ...
            final String txt;
            if (WelcomeParams.getInstance().advanceDaysHeaderToday.isEmpty()) {
                txt = "<html><p align=center><span style='font-size:22.0pt;color:red'>" + getNameWeekDay(gc.getTime()) + "<br/></span><span style='font-size:22.0pt;color:red'>" + format_dd_MMMM_yyyy.format(gc.getTime());
            } else {
                txt = String.format(WelcomeParams.getInstance().advanceDaysHeaderToday, getNameWeekDay(gc.getTime()), format_dd_MMMM_yyyy.format(gc.getTime()));
            }
            label.setText(txt);
        } else {
            final String txt;
            if (WelcomeParams.getInstance().advanceDaysHeader.isEmpty()) {
                txt = "<html><p align=center><span style='font-size:22.0pt;color:blue'>" + getNameWeekDay(gc.getTime()) + "<br/></span><span style='font-size:18.0pt;color:black'>" + format_dd_MMMM_yyyy.format(gc.getTime());
            } else {
                txt = String.format(WelcomeParams.getInstance().advanceDaysHeader, getNameWeekDay(gc.getTime()), format_dd_MMMM_yyyy.format(gc.getTime()));
            }
            label.setText(txt);
        }
    }

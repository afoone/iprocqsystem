...
public class QUserList extends ATListModel<QUser> {
    ...
    protected LinkedList<QUser> load() {
        ...
        users.stream().forEach(qUser -> qUser.setServicesCnt(qUser.getPlanServiceList().getSize()));
        return users;
    }
    ...
    private QUserList() {
        super();
        getItems().stream().forEach(qUser -> qUser.setServicesCnt(qUser.getPlanServiceList().getSize()));
    }

    @Override
    public void save() {
        deleted.stream().forEach(qUser -> qUser.setDeleted(new Date()));
